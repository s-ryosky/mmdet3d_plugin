...
from mmdet3d_plugin.core.merge_all_augs import merge_all_aug_bboxes_3d


@DETECTORS.register_module()
class UVTRKDL(MVXTwoStageDetector):
    """UVTRKDL."""
    def __init__(self,
                 distill_type='3D_2D',
                 use_grid_mask=False,
    ...
    def init_weights(self):
        ...
        if self.pretrained_img is not None:
            ...
            # load pretrained img_backbone
            if 'img_backbone' in self.load_img:
                dict_load = {_key.replace('img_backbone.',''):ckpt_load[_key] 
                            for _key in ckpt_load if 'img_backbone' in _key}
                self.img_backbone.load_state_dict(dict_load, strict=False)
                print("Loaded pretrained img_backbone")
            # load pretrained img_neck
            if 'img_neck' in self.load_img:
                dict_load = {_key.replace('img_neck.',''):ckpt_load[_key] 
                            for _key in ckpt_load if 'img_neck' in _key}
                self.img_neck.load_state_dict(dict_load, strict=False)
                print("Loaded pretrained img_neck")
    ...
    def extract_feat(self, points, img, img_metas):
        ...
        if hasattr(self, "img_backbone"):
            if self.distill_type == '3D_2D':
                img_feats = self.extract_img_feat(img, img_metas)
                img_depth = self.pred_depth(img=img, img_metas=img_metas, img_feats=img_feats)
            elif self.distill_type == '2D_3D' and self.training:
                with torch.no_grad():
                    img_feats = self.extract_img_feat(img, img_metas)
                    img_depth = self.pred_depth(img=img, img_metas=img_metas, img_feats=img_feats)
            if self.distill_type == '2D_3D' and img_feats is not None:
                img_feats = img_feats.detach()
                img_depth = img_depth.detach()
        
        if hasattr(self, 'pts_voxel_encoder'):
            if self.distill_type == '2D_3D':
                pts_feats = self.extract_pts_feat(points, img_feats, img_metas)
            elif self.distill_type == '3D_2D' and self.training:
                with torch.no_grad():
                    pts_feats = self.extract_pts_feat(points, img_feats, img_metas)
            if self.distill_type == '3D_2D' and pts_feats is not None:
                if isinstance(pts_feats, dict):
                    for _key in pts_feats:
                        if isinstance(pts_feats[_key], list):
                            pts_feats[_key] = [_feat.detach() for _feat in pts_feats[_key]]
                        else:
                            pts_feats[_key] = pts_feats[_key].detach()
                else:
                    pts_feats = pts_feats.detach()
